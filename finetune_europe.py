...
BATCH_SIZE = 8
LEARNING_RATE = 1e-5
OUTPUT_PATH = "./finbert_europe_finetuned"
...
# Load all Europe data
samples = []
for company in os.listdir(EUROPE_FOLDER):
    path = os.path.join(EUROPE_FOLDER, company, "financial_train.csv")
    if os.path.exists(path):
        df = pd.read_csv(path)
        for _, row in df.iterrows():
            samples.append({"text": row["text"], "label": int(row["label"])})

print(f"Total Europe samples: {len(samples)}")
